...
class Gestor:
    ...
    def Thread_execute(self, name_thread: str):
        ...
        main_funcion = self.hilos_registrados[name_thread]["funcion ejecutada"]
        args = self.hilos_registrados[name_thread]["args"]
        kwargs = self.hilos_registrados[name_thread]["kwargs"]

        # Le pasamos como primer parametro el evento para cancelar cuando se llame el .set() para colocarlo en TRUE.
        main_funcion(self.hilos_registrados[name_thread]["event"], *args, **kwargs)
    ...
    def Synchronous_thread(self, name_thread: str):
        if name_thread in self.hilos_registrados:
            self.hilos_registrados[name_thread]["hilo"].join()

        else:
            logging.error("No fue posible la sincronización")
    
    # Método para finalizar el hilo.
    def Thread_End(self, name_thread : str):
        if name_thread in self.hilos_registrados:
            self.hilos_registrados[name_thread]["event"].set()

        else:
            logging.error("No se pudo finalizar correctamente el hilo")

    #Funciones a utilizar
def program(event: threading.Event, program_name: str, duration: int):
    logging.info(f"Iniciando {program_name}...")
    
    for seg in range(duration):
        if event.is_set() == True:
            logging.warning("Se ha cancelado la ejecución del hilo")
            return   
        time.sleep(1)
...
if __name__ == "__main__":
    ...
    gestor.Thread_Allocate("hilo_2", program, "Proteus.exe", 7)
    gestor.Thread_Allocate("hilo_3", program, "Youtube", 9)
    ...
    # Finalizamos un hilo
    gestor.Thread_End("hilo_2")

    # Aplicamos el join() para que cada hilo termine su tarea correspondiente
    gestor.Synchronous_thread("hilo_1")
    gestor.Synchronous_thread("hilo_2")
    gestor.Synchronous_thread("hilo_3")

    logging.info("Terminando Test")
